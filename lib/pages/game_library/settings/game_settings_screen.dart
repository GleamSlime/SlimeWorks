...
class _GameSettingsScreenState
    extends BasePageState<GameLibrarySettingsViewModel, GameSettingsScreen> {
  @override
  bool get showAppBar => false;

  @override
  GameLibrarySettingsViewModel createViewModel() => GameLibrarySettingsViewModel();

  ScreenChromeData _buildChromeData() {
    return const ScreenChromeData(title: '游戏库设置');
  }
  ...
  Widget buildContent(BuildContext context) {
    return ScreenChrome(
      ...
      child: Obx(() {
        final GameLibrarySettings? settings = viewModel.settings.value;
        if (settings == null) {
            ...
            Card(
              child: SwitchListTile(
                ...
                onChanged: (bool value) {
                  viewModel.saveSettings(settings.copyWith(autoTrackPlayTime: value));
                },
              ),
            ),
            Card(
              child: SwitchListTile(
                ...
                onChanged: (bool value) {
                  viewModel.saveSettings(settings.copyWith(autoSave: value));
                },
              ),
            ),
            Card(
              child: SwitchListTile(
                ...
                onChanged: (bool value) {
                  viewModel.saveSettings(settings.copyWith(enableDesktopLaunch: value));
                },
              ),
            ),
            Card(
              child: ListTile(
                ...
                trailing: DropdownButton<String>(
                  ...
                  onChanged: (String? value) {
                    if (value != null) {
                      viewModel.saveSettings(settings.copyWith(defaultSort: value));
                    }
                  },
                ),
              ),
  ...
  Future<void> _exportBackup() async {
    ...
    final String jsonText = viewModel.exportBackupJson();
  ...
  Future<void> _importBackup() async {
    ...
    await viewModel.importBackupJson(jsonText);
    if (!mounted) {
      return;
    }
