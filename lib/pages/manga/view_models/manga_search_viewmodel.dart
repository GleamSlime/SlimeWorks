...
const String _kSearchHistoryKey = 'manga_search_history';

/// 搜索历史最大条数
const int _kSearchHistoryMaxCount = 20;

/// 搜索结果状态管理
class MangaSearchViewModel extends BaseViewModel {
  final MangaService _service = getIt<MangaService>();

  /// 搜索结果列表（含屏蔽词过滤）
  final RxList<MangaComic> results = <MangaComic>[].obs;
  ...
  /// 当前排序方式（Rx 以驱动 UI 响应）
  final Rx<MangaSortOrder> sort = MangaSortOrder.dateDescending.obs;
  ...
  Future<void> search({
    ...
    try {
      ...
      /// 加载屏蔽词配置并过滤结果
      final blockConfig = await MangaBlockWordsService.load();
      final filtered = list.comics
          .where(
            (c) => !MangaBlockWordsService.shouldBlock(
              title: c.title,
