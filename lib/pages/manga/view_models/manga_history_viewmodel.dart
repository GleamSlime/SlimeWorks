...
class MangaHistoryItem {
  ...
  factory MangaHistoryItem.fromJson(Map<String, dynamic> json) => MangaHistoryItem(
...
/// 观看记录 ViewModel
class MangaHistoryViewModel extends BaseViewModel {
  /// 当前所有观看记录（按时间倒序）
  final RxList<MangaHistoryItem> items = <MangaHistoryItem>[].obs;
  ...
  Future<void> loadHistory() async {
    setLoading(true);
    try {
      final raw = rust.mangaLoadHistory();
      if (raw.isEmpty) {
        items.clear();
        clearError();
        return;
      ...
      items.assignAll(list.map((e) => MangaHistoryItem.fromJson(e as Map<String, dynamic>)));
      clearError();
    } catch (e) {
  ...
  Future<void> _persist() async {
    ...
    rust.mangaSaveHistoryRaw(json: encoded);
  }
  ...
  static Future<void> saveRecord({
    ...
    try {
      ...
      List<MangaHistoryItem> list = [];
      if (raw.isNotEmpty) {
        final decoded = jsonDecode(raw) as List<dynamic>;
        list = decoded.map((e) => MangaHistoryItem.fromJson(e as Map<String, dynamic>)).toList();
      }

      /// 去重（同 comicId 的记录移除旧的）
      list.removeWhere((e) => e.comicId == comicId);

      final item = MangaHistoryItem(
        comicId: comicId,
        comicTitle: comicTitle,
        thumbUrl: thumbUrl,
        epsOrder: epsOrder,
