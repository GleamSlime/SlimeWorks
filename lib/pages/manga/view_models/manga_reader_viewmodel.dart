...
/// 阅读器状态管理
class MangaReaderViewModel extends BaseViewModel {
  final MangaService _service = getIt<MangaService>();

  /// 当前章节所有图片页
  final RxList<MangaPage> pages = <MangaPage>[].obs;
  ...
  /// 章节列表（用于前后章节导航）
  final RxList<MangaEps> epsList = <MangaEps>[].obs;
  ...
  static const String _kSettingsKey = 'manga_reader_settings';
  ...
  Future<void> loadPages(String comicId, int epsOrder) async {
    ...
    try {
      ...
      final pageList = results[0] as MangaPageList;
  ...
  /// 格式：manga_progress_{comicId} = '{epsOrder}:1:{epsTitle}'
  Future<void> _saveProgress() async {
    ...
    try {
      final epsTitle =
          ...
          '第$_epsOrder话';
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('manga_progress_$_comicId', '$_epsOrder:1:$epsTitle');

      /// 从元数据缓存获取漫画标题和封面（由详情页 getComicDetail 写入缓存）
      final meta = getIt<MangaService>().getComicMeta(_comicId);
      await MangaHistoryViewModel.saveRecord(
        comicId: _comicId,
