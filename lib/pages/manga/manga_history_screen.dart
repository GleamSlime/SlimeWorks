...
class _MangaHistoryScreenState extends BasePageState<MangaHistoryViewModel, MangaHistoryScreen> {
  @override
  MangaHistoryViewModel createViewModel() => MangaHistoryViewModel();
  ...
  /// 点击条目打开漫画详情页
  void _openDetail(BuildContext context, MangaHistoryItem item) {
    MangaComicDetailRoute(comicId: item.comicId).push(context);
  }
...
/// 从 thumbUrl（fileServer + /static/ + path 格式）解析出 MangaImage 并用
/// MangaImageView 渲染，保证走 Rust 带鉴权的图片请求通道。
class _ThumbImage extends StatelessWidget {
  const _ThumbImage({required this.thumbUrl});

  final String thumbUrl;

  /// 将 `fullUrl` 拆解回 MangaImage；若格式无法识别则返回 null。
  static MangaImage? _parse(String url) {
    const sep = '/static/';
