...
/// - 图片服务器选择
class MangaSettingsTab extends StatefulWidget {
  const MangaSettingsTab({super.key});

  @override
  State<MangaSettingsTab> createState() => _MangaSettingsTabState();
}

class _MangaSettingsTabState extends State<MangaSettingsTab> {
  late final MangaService _service = getIt<MangaService>();

  MangaChannelMode _channel = MangaChannelMode.direct;
  ...
  final Map<MangaChannelMode, int?> _latencyMap = {};
  ...
  Future<void> _load() async {
    ...
    final relayAddr = (channel == MangaChannelMode.lanRelay) ? customIp : '';
    setState(() {
      _channel = channel;
      _customIpCtrl.text = (channel == MangaChannelMode.cdnIp && customIp.isNotEmpty)
          ? customIp
          : _kDefaultCdnIp;
  ...
  Future<void> _applyChannel(MangaChannelMode mode) async {
    if (mode == MangaChannelMode.cdnIp && _customIpCtrl.text.trim().isEmpty) {
      _customIpCtrl.text = _kDefaultCdnIp;
    }
    setState(() => _channel = mode);
    final customIp = mode == MangaChannelMode.lanRelay
  ...
  Future<void> _testAll() async {
    ...
    setState(() {
      _testingAll = true;
      for (final m in MangaChannelMode.values) {
        _latencyMap[m] = -1; // 测速中
      }
    });

    for (final mode in MangaChannelMode.values) {
      final customIp = mode == MangaChannelMode.lanRelay
          ? _relayAddrCtrl.text.trim()
          : _customIpCtrl.text.trim();
      try {
                      ...
                      ? Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(width: m.kSpace8),
                              FilledButton.tonal(
                                onPressed: () => _applyChannel(MangaChannelMode.cdnIp),
                                child: const Text('应用'),
                              ),
                            ],
                          ),
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  SizedBox(width: m.kSpace8),
                                  FilledButton.tonal(
                                    onPressed: () => _applyChannel(MangaChannelMode.lanRelay),
                                    child: const Text('应用'),
                                  ),
                                ],
                              ),
...
class _ChannelRadioTile extends StatelessWidget {
  final MangaChannelMode mode;
  final int? latency; // null=未测, -1=测速中, -2=不可达, >=0=ms

  const _ChannelRadioTile({required this.mode, this.latency});

  static const _subtitles = {
    MangaChannelMode.direct: '标准 DNS 解析，网络环境良好时使用',
    MangaChannelMode.channel2: 'IP 直连 104.21.91.145（Cloudflare 节点，推荐）',
    MangaChannelMode.channel3: 'IP 直连 188.114.98.153（Cloudflare 节点）',
    MangaChannelMode.cdnIp: '自定义 CDN IP，可填入获取的节点地址',
    MangaChannelMode.jpProxy: '反代节点 bika-api.jpacg.cc（JP）',
    MangaChannelMode.usProxy: '反代节点 bika2-api.jpacg.cc（US）',
  };
  ...
  Widget build(BuildContext context) {
    ...
    return RadioListTile<MangaChannelMode>(
