...
const Loggers _logger = Loggers(name: 'Manga下载');

const String _kDownloadMetaKey = 'manga_downloads_meta_v1';

/// Manga 下载服务（全局单例，通过 GetIt 注入）
class MangaDownloadService {
  MangaDownloadService._();

  static final MangaDownloadService _instance = MangaDownloadService._();

  factory MangaDownloadService() => _instance;

  final MangaService _api = MangaService();

  /// 所有下载条目（comicId → entry）
  final RxMap<String, MangaDownloadEntry> entries = <String, MangaDownloadEntry>{}.obs;
  ...
  Future<void> init() async {
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        entries.value = MangaDownloadEntry.decodeAll(raw);
      }
    } catch (e) {
      _logger.error('Manga 下载服务初始化失败: $e');
    }
    _logger.info('Manga 下载服务已初始化，共 ${entries.length} 条记录');
  }

  /// ---- 外部 API ----

  /// 添加章节到下载队列并开始下载
  Future<void> downloadEps(MangaComic comic, MangaEps eps) async {
    final entry = entries.putIfAbsent(
      comic.id,
      () => MangaDownloadEntry(comicId: comic.id, comicTitle: comic.title, thumb: comic.thumb),
    );
    // 已完成则跳过
    if (entry.episodes[eps.order]?.isCompleted == true) return;

    entry.episodes[eps.order] = MangaDownloadEpsInfo(
      epsOrder: eps.order,
      epsTitle: eps.title,
  ...
  /// 批量添加章节（一次选多章）
  Future<void> downloadEpsMultiple(MangaComic comic, List<MangaEps> epsList) async {
  ...
  Future<void> retryEps(String comicId, int epsOrder) async {
    ...
    if (info == null) return;
    info.status = MangaDownloadStatus.waiting;
    info.errorMsg = null;
    entries.refresh();
  ...
  Future<void> _downloadEpsImpl(String comicId, int epsOrder) async {
    ...
    final info = entry.episodes[epsOrder]!;
    info.status = MangaDownloadStatus.downloading;
    entries.refresh();

    try {
      // 1. 获取所有图片页（支持多服务器分页）
      final allPages = <MangaPage>[];
      int page = 1;
      int totalPages = 1;
      do {
      ...
      for (int i = 0; i < allPages.length; i++) {
        ...
        try {
          ...
          _logger.error('Manga 图片下载失败 $comicId/$epsOrder/p${i + 1}: $e');
          // 单张失败不中断整体，继续下载剩余
        }
      }

      if (info.downloadedPages >= info.totalPages) {
        info.status = MangaDownloadStatus.completed;
      } else {
        info.status = MangaDownloadStatus.error;
        info.errorMsg = '部分图片下载失败（${info.downloadedPages}/${info.totalPages}）';
      }
    } catch (e) {
      info.status = MangaDownloadStatus.error;
      info.errorMsg = e.toString();
      _logger.error('Manga 章节下载失败 $comicId/$epsOrder: $e');
    }
  ...
  Future<void> _persistMeta() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(_kDownloadMetaKey, MangaDownloadEntry.encodeAll(entries));
    } catch (e) {
      _logger.error('Manga 下载元数据持久化失败: $e');
    }
  }
}
