...
const Loggers _logger = Loggers(name: 'PicACG');
...
class PicAcgService {
  ...
  Future<void> init() async {
    _logger.info('PicACG Service 初始化');
    rust.picacgInit();
    ...
    try {
      ...
      _logger.info('PicACG 历史记录数据库初始化: $dbPath');
    } catch (e) {
      _logger.error('PicACG 历史记录数据库初始化失败: $e');
    }
    ...
    try {
      ...
      if (savedProxy.isNotEmpty) {
        ...
        _logger.info('PicACG 恢复代理配置: $savedProxy');
      }
      ...
      _logger.info('PicACG 恢复分流模式: $channelVal custom=$channelCustom');
      ...
      if (savedToken.isNotEmpty) {
        ...
        try {
          ...
          _isLoggedIn = true;
          _logger.info('PicACG 恢复登录 Token 并加载用户信息成功');
        } catch (e) {
          _logger.error('PicACG 恢复登录失败，已清除失效 Token: $e');
          rust.picacgLogout();
          _isLoggedIn = false;
          _currentUser = null;
  ...
  Future<void> setProxy(String proxyUrl) async {
    ...
    _logger.info('PicACG 代理已更新: $proxyUrl');
  }
  ...
  Future<void> saveLoginCredentials({required String email, required String password}) async {
    ...
    _logger.info('PicACG 已保存登录凭据: email=${email.trim()}');
  }
  ...
  Future<void> setChannel(PicAcgChannelMode mode, {String customIp = ''}) async {
    ...
    _logger.info('PicACG 分流模式已更新: ${mode.label}, custom=$effectiveCustomIp');
  }
  ...
  Future<void> setImageServer(String server) async {
    ...
    _logger.info('PicACG 图片服务器已更新: $server');
  }
  ...
  Future<int> testChannel(PicAcgChannelMode mode, {String customIp = ''}) async {
    final effectiveCustomIp = _effectiveCustomIp(mode, customIp);
    _logger.info('PicACG 测速开始: ${mode.label} customIp=$effectiveCustomIp');
    try {
      final ms = await rust.picacgTestChannel(mode: mode.value, custom: effectiveCustomIp);
      _logger.info('PicACG 测速结果: ${mode.label} = ${ms}ms');
      return ms.toInt();
    } catch (e) {
      _logger.info('PicACG 测速失败: ${mode.label} err=$e');
      rethrow;
    }
  }
  ...
  Future<Map<PicAcgChannelMode, int>> testAllChannels({String customIp = ''}) async {
    _logger.info('PicACG 开始并行测速所有节点...');
    ...
    _logger.info('PicACG 测速完成: $result');
    return result;
  }
  ...
  Future<void> _syncNetworkConfigToRust() async {
    ...
    rust.picacgSetImageServer(server: imageServer);
    _imageCache.clear();
    _logger.info('PicACG 登录前同步网络配置: mode=${mode.label}, custom=$custom, proxy=$proxy');
  }
  ...
  Future<PicAcgUser> _finalizeLoginSuccess() async {
    ...
    _currentUser = user;
    _logger.info('PicACG 登录成功: ${user.name}');
    return user;
  }
  ...
  Future<PicAcgComicList> getComicsByCategory({
    ...
    _logger.info('PicACG 按分类: $category, 第$page页');
  ...
  Future<PicAcgComicList> searchComics({
    ...
    _logger.info('PicACG 搜索: "$keyword", 第$page页');
  ...
  Future<List<PicAcgComic>> getRankings(String timeType) async {
    _logger.info('PicACG 排行榜: $timeType');
  ...
  Future<PicAcgComic> getComicDetail(String comicId) async {
    _logger.info('PicACG 漫画详情: $comicId');
  ...
  Future<PicAcgEpsList> getComicEps(String comicId, {int page = 1}) async {
    _logger.info('PicACG 章节列表: $comicId, 第$page页');
  ...
  Future<PicAcgPageList> getEpsPages(String comicId, int epsOrder, {int page = 1}) async {
    _logger.info('PicACG 章节图片: $comicId, 第$epsOrder集, 第$page页');
  ...
  Future<PicAcgComicList> getFavourites({
    ...
    _logger.info('PicACG 收藏列表: 第$page页');
