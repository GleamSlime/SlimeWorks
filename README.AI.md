...
- 核心重要逻辑 flutter应该只负责ui显示和ui相关数据处理。rust负责逻辑运算、数据存储、物理设备、扫描、文件处理等。禁止职责越界！
- 应用包含模块管理系统，RUST中分为动态加载(仅桌面端)和静态链接
- 如果需求有创建新的模块参考目录如下
...
- 状态管理(UI/页面状态 使用GetX | 业务逻辑Service、Rust FFI桥接层、配置、数据库 使用GetIt)，路由操作使用GoRouter TypedGoRoute
...
- 注释必须使用中文
- 完成代码调整后需 flutter run 以校验是否有编译错误，如果只有Rust调整应当cargo build，随后完善本次修改的测试用例代码
- 完成代码编写后在docs文件夹下面输出Markdown，如果该模块下存在文档则更新完善文档，文档因从介绍模块考虑
