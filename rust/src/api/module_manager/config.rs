...
pub fn build_filename(version_info: &VersionInfo, module_type: &ModuleType) -> String {
    let add_lib_prefix = matches!(module_type, ModuleType::DynamicLibrary | ModuleType::Library)
...
mod tests {
    ...
    fn test_build_filename() {
        let version_info = VersionInfo::new("capture_proxy".to_string(), "1.2.0+23".to_string(), false);
